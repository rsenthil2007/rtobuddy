 ...
 *   1. Add a function here: `function myScene() { return '<svg>…</svg>'; }`
...
/**
 * Returns an HTML string for the animation, or '' if no animation matches.
 * @param {string} animationType - key from the ANIMATIONS registry
 * @returns {string} SVG HTML or empty
 */
export function renderAnimation(animationType) {
  const builder = ANIMATIONS[animationType];
  if (!builder) return '';
  return `<div class="anim-scene">${builder()}</div>`;
}

export function hasAnimation(animationType) {
  return Boolean(ANIMATIONS[animationType]);
}
